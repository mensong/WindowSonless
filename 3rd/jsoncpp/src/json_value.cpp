...
bool Value::operator<(const Value& other) const {
  ...
  case arrayValue:
  case objectValue: {
	if (value_.map_ == NULL || other.value_.map_ == NULL) {
...
const char* Value::asCString() const {
  ...
  if (value_.string_ == 0 || !isString()) return 0;
  unsigned this_len;
  char const* this_str;
  decodePrefixedString(this->allocated_, this->value_.string_, &this_len, &this_str);
  return this_str;
...
bool Value::isConvertibleTo(ValueType other) const {
  ...
  case nullValue:
    return (isNumeric() && asDouble() == 0.0) ||
           ...
           (type_ == arrayValue && value_.map_ != NULL && value_.map_->size() == 0) ||
           (type_ == objectValue && value_.map_ != NULL && value_.map_->size() == 0) ||
           type_ == nullValue;
  case intValue:
    return isInt() ||
...
Value& Value::operator[](ArrayIndex index) {
  ...
  if (type_ == nullValue || value_.map_ == NULL)
    *this = Value(arrayValue);
  CZString key(index);
...
{
  if (type_ != objectValue || value_.map_ == NULL) {
    return false;
  }
  CZString actualKey(key, static_cast<unsigned>(cend-key), CZString::noDuplication);
